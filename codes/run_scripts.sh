...
# python -m torch.distributed.launch --nproc_per_node=4 --master_port=4322 train.py -opt options/train/train_EDVR_woTSA_Youku.yml --launcher pytorch 
# python -m torch.distributed.launch --nproc_per_node=4 --master_port=4322 train.py -opt options/train/train_EDVR_Youku.yml --launcher pytorch